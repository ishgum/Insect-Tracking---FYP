...
KalmanFilter setKalmanParameters(void) {
...
int main(int argc, char** argv)
{
	...
	while (!src.empty()) {
		...
		//Get measurements
		if (insect.found) {
			measurement.at<float>(0) = insect.position.x; //mc[0].x + float(ROI.x);
			measurement.at<float>(1) = insect.position.y; //mc[0].y + float(ROI.y);
		}
		else {
