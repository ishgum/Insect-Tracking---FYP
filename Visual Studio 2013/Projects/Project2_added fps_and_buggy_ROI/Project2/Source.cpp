/* This program performs tracking on a bright object.
Created by Michael McAdam, Daniel Morris, and Dylan Mackie
*/
#define RUN
#ifdef RUN
...
//const string videoFile = "C:/Users/myadmin/Documents/IR footage/retro2_2015-05-09-193310-0000.avi"; //try another day

#define OVERALL_SCALE_X		0.3  // Simple global downsampling for performance, fraction /1
#define OVERALL_SCALE_Y		0.3
#define ROI_SIZE_X			500//576 // Size of first finding attempt region, #pixels
#define ROI_SIZE_Y			300//324 // 576 324
#define NUM_WEIRD_FRAMES	40 // number of frames to process before using ROI because bugs
#define FRAME_MS			40
...
Mat initial_filtering(Mat raw_source){
	Mat dst, hsl;
	Mat values[3]; 
	//t raw_source = raw_source2.clone(); 

	static int lumThreshold = 0;


	// Convert image to HSL
	cvtColor(raw_source, hsl, CV_BGR2HLS);

	//drawCross(raw_source, Point(100,100), Scalar(255, 255, 255),5); // used to test 
...
int main(int argc, char** argv)
{
	...
	while (!src.empty()) {
		...
		bool bug_found_in_frame = false;

		//Check if we are in inital find period:

		if (frame_num > NUM_WEIRD_FRAMES){
			src_ROI = src(ROI);
			//src_ROI = src;
			dst = initial_filtering(src_ROI); // does all the early steps. only pointers are passed to & from this function
			//vector<vector<Point> > contours;
			//vector<Vec4i> hierarchy;
			...
			if (contours.size() >= 1){
				//FOUND CONTOURS YAY, continue with current ROI
				bug_found_in_frame = true;
			}
		}

			else{
				// Make ROI the full image & repeat
		/*		ROI.x = 0;
				ROI.y = 0;
				ROI.width = src.cols;
				ROI.height = src.rows;*/
				//src_ROI = src(ROI);
				src_ROI = src;
				imshow("Bug not found (last frame)", src_ROI);
				dst = initial_filtering(src_ROI); // does all the early steps. only pointers are passed to & from this function
				///vector<vector<Point> > contours;
				//vector<Vec4i> hierarchy;
				Canny(dst, dst, 100, 100 * 2, 3);
		...
		/// Show in a window
		// also display number of contours found in window
		//Frame Number
		char contour_num_c[5];
		sprintf(contour_num_c, "Contours #: %5i", useContours.size());
		Point contour_num_text_loc(10, 30);
		putText(drawing, contour_num_c, contour_num_text_loc,
			FONT_HERSHEY_SIMPLEX, 0.6, { 255, 255, 255 }, 1.5);
		...
		printf("\nxy_Loc: %f x  and  %i y", predict.at<float>(0), xy_loc.y);
		...
		if (useContours.size() >= 1) { // dylan changed this to 2, from 1 // back to 1
			float xcont = mc[0].x;
			float ycont = mc[0].y;
			printf("\nMomentLoc: %f x  and  %f y", xcont, ycont);

			measurement.at<float>(0) = mc[0].x + (float)padleft;// +padleft; // changed to provide abs reference
			measurement.at<float>(1) = mc[0].y + (float)padtop;// +padtop; // changed to provide abs reference
		}
		else {
		...
		printf("\nstateLoc: %i and %i", stateLoc.x, stateLoc.y);
		...
		printf("\nPadleft: %i Padtop %i", padleft, padtop);
		...
		if (!src.empty()) {
			frame_num++;
			resize(src, src, Size(), OVERALL_SCALE_X, OVERALL_SCALE_Y);
			wait_period = FRAME_MS; //change me so timer function can account for it. chee
			waitKey(wait_period);
		}
	}
